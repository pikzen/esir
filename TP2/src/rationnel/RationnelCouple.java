...
public class RationnelCouple implements Rationnel {
	...
	/**
	 * Construit un RationnelCouple avec numÃ©rateur et dÃ©nominateur
	 * 
	 * @param num
	 *            NumÃ©rateur du rationnel
	 * @param den
	 *            DÃ©nominateur du rationnel
	 */
	...
	public Rationnel somme(Rationnel r2){
		Rationnel result;
		// Si le dénominateur est égal, on peut simplement ajouter les
		// numérateurs et renvoyer le résultat.
		// Sinon, on doit multiplier numérateurs et dénominateurs entre eux,
		// etc.
		if (this.getDenominateur() == r2.getDenominateur()) {
			result = new RationnelCouple(this.getNumerateur()
					+ r2.getNumerateur(), this.getDenominateur());
		} else {
			...
			while (den1 != den2) {
				if (den1 < den2)
					den1 = den1 + this.getDenominateur();
				else
					den2 = den2 + r2.getDenominateur();
			}
			int factor1 = den1 / this.getDenominateur();
			int factor2 = den2 / r2.getDenominateur();
			int num = this.getNumerateur() * factor1 + r2.getNumerateur()
					* factor2;
			int den = den1;
